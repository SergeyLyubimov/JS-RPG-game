...
export default class Player {
  ...
  takeAttack(damage) {
    if (this.isAttackBlocked()) {
      ...
      console.log(`${this.name} блокирует! Его оружие ${this.weapon.name} получает ${damage} урона.`);
      this.checkWeapon();
    } else if (!this.dodged()) {
      this.takeDamage(damage);
      console.log(`${this.name} получает ${damage} урона.`);
    } else {
      console.log(`${this.name} уворачивается!`);
    }
  }
  ...
  checkWeapon() {
    if (this.weapon.isBroken() & !(this.weapon instanceof Knife)) {
      ...
      console.log(`Оружие ${oldWeaponName} у ${this.name} ломается! Он берет в руки ${this.weapon.name}.`);
    } else if (this.weapon.isBroken()) {
      const oldWeaponName = this.weapon.name;
      this.weapon = new Arm;
      console.log(`Оружие ${oldWeaponName} у ${this.name} ломается! У него остались только Руки.`);
    }
  }
